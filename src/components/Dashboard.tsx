...
const Dashboard: React.FC<DashboardProps> = ({ users, currentUser, onLogout }) => {
  ...
  const sortedUsers = [...filteredUsers].sort((a, b) => {
    if (sortBy === 'username') {
      ...
      return new Date(b.lastPasswordChange).getTime() - new Date(a.lastPasswordChange).getTime();
    }
  });
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {users.filter(u => getPasswordStrength(u.passwordHash) === 'Strong').length}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {users.filter(u => {
                    const daysSince = (Date.now() - new Date(u.lastPasswordChange).getTime()) / (1000 * 60 * 60 * 24);
                    return daysSince <= 7;
                  }).length}
                </p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {Math.round((users.filter(u => getPasswordStrength(u.passwordHash) === 'Strong').length / users.length) * 100)}%
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex gap-4 items-end">
              <div>
                ...
                <select
                  id="sort"
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value as 'username' | 'date')}
                  ...
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        getPasswordStrength(user.passwordHash) === 'Strong' ? 'bg-green-100 text-green-800' :
                        getPasswordStrength(user.passwordHash) === 'Good' ? 'bg-blue-100 text-blue-800' :
                        getPasswordStrength(user.passwordHash) === 'Fair' ? 'bg-yellow-100 text-yellow-800' :
